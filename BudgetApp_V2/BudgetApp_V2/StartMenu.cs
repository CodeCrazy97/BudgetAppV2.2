...
namespace BudgetApp_V2
{
    public partial class StartMenu : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            GoogleSheetsService sheetsService = new GoogleSheetsService(apiKey, spreadsheetId);
            IList<IList<object>> data = sheetsService.ReadAllRowsFromSheet();
            ...
            if (data != null && data.Count > 0)
            {
                ...
                {
                    if (currentRow == 0) // skip first row - it's the header
                    {
                        currentRow++;
                        continue;
                    } else
                    ...
                    foreach (var cell in row)
                    {
                        ...
                        DateTime inputDate = DateTime.ParseExact(row[0].ToString(), "M/d/yyyy", CultureInfo.InvariantCulture);

                        // Format the DateTime object into the MySQL date format
                        string mysqlDateFormat = inputDate.ToString("yyyy-MM-dd");
                        if (!mySQLConnection.CheckIfTransactionExists(connection, mysqlDateFormat, row[1].ToString(), Double.Parse(row[2].ToString()), row[3].ToString().ToLower()))
                        {
                            if (!AddTransactionToDB(connection, mysqlDateFormat, row[1].ToString(), Double.Parse(row[2].ToString()), row[3].ToString().ToLower()))
                            {
                                MessageBox.Show("Transaction could not be added! Date: " + mysqlDateFormat + ", description: " + row[1].ToString() + ", amount: " + Double.Parse(row[2].ToString()) + ", category: " + row[3].ToString());
                                connection.Close();
                                return;
                            }
                        }
                ...
                MessageBox.Show("Finished syncing with Google! Synced " + (currentRow - 1) + " transaction(s).");
                connection.Close();
            }
        }

        public bool AddTransactionToDB(MySqlConnection connection, string date, string description, double amount, string category)
        {
            ...
            {
                MessageBox.Show("The date " + date + " could not be formatted correctly for the database: " + fe.Message);
            }

            if (mySqlFixedDate.Equals(""))
            {
                MessageBox.Show("The date " + date + " could not be converted to a MySQL date.");
            }

            string sql = "";
            if (category == "other earnings")
            {
                sql = "INSERT INTO other_earnings (earning_date, amount, description, expense_type) VALUES (@trans_date, @amount, @description, @expense_type);";
            } else if (category == "charity")
            {
                sql = "INSERT INTO charity (trans_date, amount, description) VALUES (@trans_date, @amount, @description);";
            } else
            {
                sql = "INSERT INTO expenses (trans_date, amount, description, expense_type) VALUES (@trans_date, @amount, @description, @expense_type);";
            }
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@expense_type", category);
                var reader = cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
                success = false;
                MessageBox.Show("Error trying to add transaction(s): " + ex.Message);
            }
